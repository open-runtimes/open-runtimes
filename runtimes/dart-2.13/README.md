...
This is the Open Runtime that builds and runs Dart code based on a `dart:2.13` base image. 
...
```bash
docker run -e ENTRYPOINT_NAME=main.dart --rm --interactive --tty --volume $PWD:/usr/code openruntimes/dart:2.13 sh /usr/local/src/build.sh
```

3. Spin-up open-runtime:

```bash
docker run -p 3000:3000 -e INTERNAL_RUNTIME_KEY=secret-key -e INTERNAL_RUNTIME_ENTRYPOINT=main.dart --rm --interactive --tty --volume $PWD/code.tar.gz:/tmp/code.tar.gz:ro openruntimes/dart:2.13 sh /usr/local/src/start.sh
```
...
```bash
cd open-runtimes/runtimes/dart-2.13
```
