...
```bash
docker run --rm --interactive --tty --volume $PWD:/usr/code open-runtimes/dart:2.13 sh /usr/local/src/build.sh
```

3. Spin-up open-runtime:

```bash
docker run -p 3000:3000 -e INTERNAL_RUNTIME_KEY=secret-key --rm --interactive --tty --volume $PWD/code.tar.gz:/tmp/code.tar.gz:ro open-runtimes/dart:2.13 sh /usr/local/src/start.sh
```
