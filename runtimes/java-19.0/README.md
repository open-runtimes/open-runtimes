...
This is the Open Runtime that builds and runs Java code based on a `openjdk:19-jdk-slim` base image. 
...
```bash
docker run -e INTERNAL_RUNTIME_ENTRYPOINT=Index.java --rm --interactive --tty --volume $PWD:/usr/code openruntimes/java:v2-19.0 sh /usr/local/src/build.sh
```

3. Spin-up open-runtime:

```bash
docker run -p 3000:3000 -e INTERNAL_RUNTIME_KEY=secret-key --rm --interactive --tty --volume $PWD/code.tar.gz:/tmp/code.tar.gz:ro openruntimes/java:v2-19.0 sh /usr/local/src/start.sh
```
...
```bash
cd open-runtimes/runtimes/java-19.0
```
