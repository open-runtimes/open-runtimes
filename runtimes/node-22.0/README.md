...
This is the Open Runtime that builds and runs NodeJS code based on a `node:22-alpine3.18` base image.
...
```bash
docker run --rm --interactive --tty --volume $PWD:/mnt/code:rw -e OPEN_RUNTIMES_ENTRYPOINT=index.js openruntimes/node:v4-22.0 sh helpers/build.sh "npm install"
```

3. Spin-up open-runtime:

```bash
docker run -p 3000:3000 -e OPEN_RUNTIMES_SECRET=secret-key --rm --interactive --tty --volume $PWD/code.tar.gz:/mnt/code/code.tar.gz:ro openruntimes/node:v4-22.0 sh helpers/start.sh "pm2 start src/server.js --no-daemon"
```
...
```bash
cd open-runtimes/runtimes/node-22.0
```
