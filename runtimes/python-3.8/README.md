...
This is the Open Runtime that builds and runs Python code based on a `python:3.8.12-alpine3.15` base image. 
...
```bash
docker run --rm --interactive --tty --volume $PWD:/usr/code open-runtimes/python:3.8 sh /usr/local/src/build.sh
```

3. Spin-up open-runtime:

```bash
docker run -p 3000:3000 -e INTERNAL_RUNTIME_KEY=secret-key --rm --interactive --tty --volume $PWD/code.tar.gz:/tmp/code.tar.gz:ro open-runtimes/python:3.8 sh /usr/local/src/start.sh
```
...
2. Enter the Python 3.8 runtime folder:
