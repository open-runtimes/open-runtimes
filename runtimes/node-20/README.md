...
This is the Open Runtime that builds and runs NodeJS code based on a `node:20-alpine` base image. 
...
```bash
docker run --rm --interactive --tty --volume $PWD:/usr/code openruntimes/node:v2-20 sh /usr/local/src/build.sh
```

3. Spin-up open-runtime:

```bash
docker run -p 3000:3000 -e INTERNAL_RUNTIME_KEY=secret-key -e INTERNAL_RUNTIME_ENTRYPOINT=index.js --rm --interactive --tty --volume $PWD/code.tar.gz:/tmp/code.tar.gz:ro openruntimes/node:v2-20 sh /usr/local/src/start.sh
```
...
```bash
cd open-runtimes/runtimes/node-20
```
