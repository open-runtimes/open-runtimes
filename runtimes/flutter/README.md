...
- To make Appwrite-defined function/site vars available to `flutter build` as compile-time `String.fromEnvironment` values, set them as regular env vars on the build container and list their names (space-separated) in `OPEN_RUNTIMES_BUILD_KEYS` (e.g. `-e API_URL=https://example.com -e DB_URL=postgresql://localhost/db -e OPEN_RUNTIMES_BUILD_KEYS="API_URL DB_URL"`). The runtime reads each listed var from the environment into its own `dart_defines.json` and transparently injects `--dart-define-from-file` into any `flutter build` call, so this works regardless of what `buildCommand` you set — including a custom script. If your project already commits its own `dart_defines.json`, it's passed to `flutter build` as a second `--dart-define-from-file`; Flutter resolves conflicts itself, and your file's values always win.
