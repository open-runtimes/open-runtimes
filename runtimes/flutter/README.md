...
- To make Appwrite-defined function/site vars available to `flutter build` as compile-time `String.fromEnvironment` values, pass them as a JSON object via `OPEN_RUNTIMES_BUILD_VARS` during build (e.g. `-e OPEN_RUNTIMES_BUILD_VARS='{"API_URL":"https://example.com"}'`). The runtime merges them into `dart_defines.json` and transparently injects `--dart-define-from-file` into any `flutter build` call, so this works regardless of what `buildCommand` you set — including a custom script. If your project already commits its own `dart_defines.json`, values already in that file always take priority over `OPEN_RUNTIMES_BUILD_VARS`.
