...
This is the Open Runtime that builds and runs PHP code based on a `php:8.1.3RC1-cli-alpine` base image. 
...
```bash
docker run --rm --interactive --tty --volume $PWD:/usr/code open-runtimes/php:8.1 sh /usr/local/src/build.sh
```

3. Spin-up open-runtime:

```bash
docker run -p 3000:3000 -e INTERNAL_RUNTIME_KEY=secret-key --rm --interactive --tty --volume $PWD/code.tar.gz:/tmp/code.tar.gz:ro open-runtimes/php:8.1 sh /usr/local/src/start.sh
```
