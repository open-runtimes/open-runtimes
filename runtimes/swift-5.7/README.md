...
This is the Open Runtime that builds and runs NodeJS code based on a `swiftarm/5.7.2-focal-multi-arch` base image. 
...
```bash
docker run --rm --interactive --tty --volume $PWD:/usr/code openruntimes/swift:v2-5.7 sh /usr/local/src/build.sh
```

3. Spin-up open-runtime:

```bash
docker run -p 3000:3000 -e INTERNAL_RUNTIME_KEY=secret-key --rm --interactive --tty --volume $PWD/code.tar.gz:/tmp/code.tar.gz:ro openruntimes/swift:v2-5.7 sh /usr/local/src/start.sh
```
...
```bash
cd open-runtimes/runtimes/swift-5.7
```
