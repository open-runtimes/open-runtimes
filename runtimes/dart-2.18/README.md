...
This is the Open Runtime that builds and runs Dart code based on a `dart:2.18` base image. 
...
```bash
docker run -e OPEN_RUNTIMES_ENTRYPOINT=lib/main.dart --rm --interactive --tty --volume $PWD:/mnt/code openruntimes/dart:v4-2.18 sh helpers/build.sh
```

3. Spin-up open-runtime:

```bash
docker run -p 3000:3000 -e OPEN_RUNTIMES_SECRET=secret-key --rm --interactive --tty --volume $PWD/code.tar.gz:/mnt/code/code.tar.gz:ro openruntimes/dart:v4-2.18 sh helpers/start.sh "/usr/local/server/src/function/server"
```
...
```bash
cd open-runtimes/runtimes/dart-2.18
```
