# Python ML Runtime 3.14

This is the Open Runtime that builds and runs Python ML code based on a `python:3.14-bookworm` base image. 
...
```bash
docker run -e OPEN_RUNTIMES_ENTRYPOINT=main.py --rm --interactive --tty --volume $PWD:/mnt/code openruntimes/python-ml:v5-3.14 sh helpers/build.sh
```

3. Spin-up open-runtime:

```bash
docker run -p 3000:3000 -e OPEN_RUNTIMES_SECRET=secret-key --rm --interactive --tty --volume $PWD/code.tar.gz:/mnt/code/code.tar.gz:ro openruntimes/python-ml:v5-3.14 sh helpers/start.sh "python3 src/server.py"
```
...
```bash
cd open-runtimes/runtimes/python/versions/ml-3.14
```
