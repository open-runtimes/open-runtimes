...
docker run --rm \
  ...
  -v "$(pwd)"/code.tar.gz:/usr/code/code.tar.gz:rw \
  -e OPEN_RUNTIMES_ENTRYPOINT="${ENTRYPOINT}" open-runtimes/test-runtime \
  sh -c "tar -xzf /usr/code/code.tar.gz -C /usr/code && sh /usr/local/src/build.sh"
...
docker run --rm -d \
  ...
  sh -c "cp /usr/code/code.tar.gz /tmp/code.tar.gz && sh /usr/local/src/start.sh"
